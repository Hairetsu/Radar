...
- Separate AI Operator window: keep the workspace at full width while a singleton, non-modal companion owns saved-scope prompting, the durable agent feed, run history, Mission Graph, capability review, recovery, memory, and AI connection settings. The workspace retains a compact safety bar with Pause, Resume, Stop, and Open/Focus AI Operator.
