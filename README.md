...
- Separate AI Operator window: keep the workspace at full width while a singleton, non-modal companion owns saved-scope prompting, one sequential browser operator, a full-window newest-first live stream with a pinned Thoughtstream, overlay run history and inspection drawers, focused capability permission prompts, recovery, memory, and AI connection settings. The workspace retains a compact safety bar with Pause, Resume, Stop, and Open/Focus AI Operator.
...
The companion's full-window feed and fixed composer hold goal setup, profiles, optional **Tutorial Mode**, budget chips, Thoughtstream decisions, tool results, policy blocks, recovery, finding and memory proposals. The Thoughtstream remains visible while the durable transcript scrolls independently with the newest event at the top. Live-follow stays at that edge as events arrive, pauses when the operator scrolls down to inspect history, and exposes **Follow latest** to return immediately. Run History and Mission Inspector open from persistent header controls as overlay drawers; the inspector holds the Mission Graph, capabilities, findings, memory, and Connection settings. Permission requests interrupt with an exact-bounds Grant/Deny dialog instead of being buried in transcript history. The window is a singleton: repeated open requests focus it, closing hides it without pausing or stopping a run, and reopening restores the current run and section.

Profiles include Browser Assessment, Passive Map, Auth Review, API Hardening, Header/Cookie Review, Advanced API Review, and Report From Evidence. Browser Assessment is the default live-site path: clicking **Start Run** confirms tightly bounded, `GET`-only opening and navigation inside saved Scope. One planner loop inspects the visible page, chooses one task-relevant unvisited in-scope path, navigates or clicks it, lets network activity settle, receives the new captures, and only then selects the next path or continues into analysis. Its effect-bearing active runtime budget is 10 minutes. Time spent waiting for provider inference does not consume that budget; navigation, tool execution, replay, and workflows still do. Form interaction, identity changes, replay, and workflow execution retain separate capability review. Every profile constrains tools and budgets for steps, replay, workflow requests, capture sample, timeout, and raw-context policy. Selecting a saved run swaps the graph, Thoughtstream, transcript, findings, status, budgets, and controls. **Pause** stores a durable checkpoint; **Resume** continues from it while keeping active elapsed time, tool steps, replay sends, and workflow requests cumulative instead of resetting the profile budget. When a runtime or tool-call budget is spent, Radar seals that checkpoint, disables Resume, and offers **Continue as New Run**; the new run receives a fresh bounded profile budget while the source transcript remains preserved and linked in the continuation timeline.

Radar does not launch a separate recon wave or parallel model workers. The same single operator performs discovery sequentially in the visible browser. Each browser action settles before the next planner decision, and newly attributed in-scope captures are included automatically in that decision context. The operator tracks visited paths from browser state, captured traffic, and the durable timeline to avoid loops, finishes task-relevant traversal, then continues into focused analysis or bounded verification.
...
Every run has a revisioned capability ledger stored locally. When the planner selects a gated tool without matching authority, Radar derives the smallest lease request from the normalized action instead of trusting provider-authored bounds: one tool, its fixed minimum risk tier, the exact origin + method + path-prefix + identity tuple, one use, known request cost, concurrency, and exact payload size. Malformed optional provider lease metadata therefore cannot fail the run or widen authority. Radar automatically grants only an in-scope, `GET`-only `openBrowser` or `navigateBrowser` lease because the operator already confirmed that narrow authority with **Start Run** or **Start Tutorial**; the grant and every use still appear in the ledger and transcript. Other derived lease requests remain drafts: Radar records the pending call, pauses before dispatch, and opens a focused permission dialog containing every bound and the reason. **Grant Exact Bounds** grants only that draft and never resumes automatically; **Deny** revokes it. After the operator clicks **Resume**, the saved pending call is attempted exactly once before planning continues; a block or failure pauses again for review instead of silently retrying. Operators can also **Pause** first to propose, grant, deny, or revoke leases manually. Completed and stopped ledgers are read-only.

Before a leased action dispatches, Radar reserves its action and known explicit-request cost in a durable receipt; afterward it finalizes the receipt as succeeded, failed, or unknown. Expiry or exhaustion blocks further use. Saved-scope drift, auth-state drift before a later dispatch, an out-of-scope resulting URL, a failed/unknown outcome, operator revocation, run stop/completion, or runtime/session change revokes granted authority. Successful browser actions may legitimately follow in-scope redirects or rotate cookies/storage; Radar records the success and binds the next lease to the resulting state instead of turning that expected effect into a recovery pause. Lease accounting covers normalized leased actions and known explicit costs, including declared active-workflow request cost. C10 adds causal attribution for observed managed-Chrome traffic, but attributed browser subresources do not become additional lease receipt costs.
...
- Asymmetric layout: compact grouped navigation, a sidebar Console block for app-global controls, a classification banner up top, restrained display numerals anchoring each panel, dense evidence grids, a full-width workspace mission bar, a full-window AI Operator live stream with overlay drawers, and a bottom telemetry ticker.
