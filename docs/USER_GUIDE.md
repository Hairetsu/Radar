...
4. Click **Approve Once**, **Approve All**, or **Deny** in the dialog. **Approve Once** retains the displayed exact path and one-use bounds. **Approve All** retains the same tool, origin, method, and identity but covers matching paths on that origin up to the selected run profile's duration, action, and request ceilings. Saved Scope, auth binding, remaining run budgets, and destructive-action blocks still apply. Approval only changes the lease ledger; it never resumes or dispatches the pending call automatically.
...
When a run starts, Radar records a full observation transcript. One sequential operator chooses a browser path or tool action, observes the settled result and newly captured evidence, then chooses the next action or returns `finish`. The compact **Mission Pulse** stays visible above the stream and updates the current mission focus, concise planner rationale, selected tool, visible target, and latest result as each decision is saved. The full-width **Operation Stream** below it is newest-first and groups each new run operation into **Decide → Act → Observe** while retaining every underlying durable audit event. Radar does not fall back to a preset autonomous script if the configured AI planner fails. The saved transcript is not truncated: it keeps system markers, Mission Graph revisions, capability decisions and receipts, rationale summaries, tool calls, tool results, visible targets, policy blocks, failed steps, timings, and recovery actions. Use **Audit detail** to reveal those identifiers, **Runs** to select any saved run in the active session, and **Inspect** to review its graph, lease ledger, findings, memory, status, and budgets without permanently reducing stream width.
...
- A gated action outside the narrow auto-granted navigation class creates a Radar-derived draft and pauses with the exact pending call before dispatch. **Approve Once** keeps the exact path and one-use bounds. **Approve All** covers matching calls only for the same tool, origin, method, and identity, remains profile- and budget-capped, and is invalidated by Scope or auth drift. Neither approval resumes automatically; after a separate **Resume**, the saved pending call is attempted exactly once.
