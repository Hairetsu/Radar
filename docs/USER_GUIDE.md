...
- Detects an orphaned Radar-owned Chrome left by an interrupted app restart, proves ownership from the exact profile and browser process, closes that stale instance, and relaunches it against the current proxy. Unrelated Chrome profiles are never closed.
...
The AI Operator gives the full companion window to its live stream and bottom command deck. Before a run, the deck holds the goal, profile, Tutorial Mode, and start controls. While a run is active, it collapses to a compact mission summary, budgets, and lifecycle controls so the stream gains the available height. The compact deck keeps a visible **Pause & Steer** action: it waits for the current tool to settle, checkpoints the run, and restores the direction editor. Enter what to prioritize, avoid, or investigate next, then click **Add Direction**. Radar preserves the original goal in the audit trail and records the update as a reviewed Mission Graph objective. Paused and failed runs expose the same editor directly.

The compact **Mission Pulse** answers what Radar is doing, why it selected the step, where it is acting, and whether execution is working, waiting, blocked, or complete. It is an auditable decision brief, not private model chain-of-thought. The separately scrolling **Operation Stream** presents the newest operation first and groups related durable events into **Decide → Act → Observe**. The live operation and newest completed operation open automatically; older successful operations collapse to a summary row, while failures, policy blocks, permission requirements, and recovery choices remain expanded. Click an operation header to expand or collapse it. **Audit detail** exposes durable entry, action, receipt, timing, and operation identifiers without replacing the readable summary.

Scrolling down into older history pauses live-follow so reading is not interrupted. If activity arrives while you review history, the return control shows the unseen count; click it to resume following the newest operation. A target's **Preview** action opens a sanitized companion drawer containing only persisted timeline metadata. Request bodies, raw headers, cookies, and storage values remain in the authorized workspace surfaces; click **Reveal in Workspace** to inspect the target through the existing scope-checked workspace path. On desktop, full-height **Task History** stays beside the stream and can collapse to a narrow status rail; Radar remembers that desktop choice locally. Use **New Task**, search, and the status-marked task cards to start a clean prompt or revisit any saved task without opening another drawer. Selecting history changes the visible transcript and inspector only; it does not restart or replay the task. In compact windows, **Tasks** opens the same history as an overlay so the operation stream keeps the available width. **Inspect** continues to open Mission Inspector as an overlay. Memory proposals and recovery controls remain attached to the operation that produced them, while draft findings remain visible after the operation list. Capability drafts still open a focused Approve/Deny permission dialog so the decision cannot be buried in history. The companion is a singleton: repeated open requests focus the same window, closing hides it without pausing or stopping a run, and reopening restores the selected section and durable run state.
...
Each run owns a locally persisted, revision-checked capability ledger containing draft, granted, revoked, expired, or exhausted leases plus durable action receipts. Choosing another task in **Task History** swaps to that run's ledger. Completed and stopped ledgers are read-only.
...
#### Radar-Derived Proposal And Approval Flow
...
4. Click **Approve Once**, **Approve All**, or **Deny** in the dialog. **Approve Once** retains the displayed exact path and one-use bounds. **Approve All** retains the same tool, origin, method, and identity but covers matching paths on that origin up to the selected run profile's duration, action, and request ceilings. **Resume after approval** is selected by default. With it selected, Radar saves the grant, revalidates Scope, auth binding, remaining run budgets, operator questions, and destructive-action blocks, then queues the saved pending call exactly once.
5. Clear **Resume after approval** before approving when you want the run to remain paused for further review or authority changes. Click **Resume** when ready. On a successful resumed action Radar clears the pending call before normal planning continues; a block or failure pauses again for review instead of silently retrying.

To create or change authority yourself, click **Pause** and wait for the active tool to settle. You can then use a visible lease template to propose exact authority, approve or deny a draft, or revoke an existing lease. Ledger mutations are revision-checked. Leave **Resume after approval** selected to continue immediately from the durable checkpoint, or clear it to review the resulting ledger before clicking **Resume** yourself.
...
When a run starts, Radar records a full observation transcript. One sequential operator chooses a browser path or tool action, observes the settled result and newly captured evidence, then chooses the next action or returns `finish`. The compact **Mission Pulse** stays visible above the stream and updates the current mission focus, concise planner rationale, selected tool, visible target, and latest result as each decision is saved. The **Operation Stream** below it is newest-first and groups each new run operation into **Decide → Act → Observe** while retaining every underlying durable audit event. Radar does not fall back to a preset autonomous script if the configured AI planner fails. The saved transcript is not truncated: it keeps system markers, Mission Graph revisions, capability decisions and receipts, rationale summaries, tool calls, tool results, visible targets, policy blocks, failed steps, timings, and recovery actions. Use **Audit detail** to reveal those identifiers, **Tasks** or the adjacent Task History sidebar to select any saved run in the active session, and **Inspect** to review its graph, lease ledger, completion report, memory, status, and budgets.

When the planner returns `finish`, Radar places a durable **Completion Report** above the Operation Stream and under **Inspect → Report**. It includes the executive summary, assessed scope, bounded methodology, evidence-backed observations (including meaningful negative results), every quality-gated draft finding with affected assets, reproduction, severity rationale, remediation and uncertainties, explicit coverage limitations, and recommended next actions. A zero-finding report says that no evidence-backed draft passed the quality gate; it does not claim the target is vulnerability-free. Invalid observation citations are removed at the completion boundary, rejected draft findings remain counted, and older saved completed runs receive a conservative write-up reconstructed from their Mission Graph, retained findings, and audit trail.
...
The existing views remain visible evidence panes, so you can watch the agent use the app instead of waiting for an opaque background job. In standard AI-First mode, successful scoped inspection steps continue automatically. Capability approval also continues automatically when the default-on **Resume after approval** control remains selected. **Resume** remains available after an operator pause, question, recoverable failure, or approval made with that control cleared. Tutorial Mode pauses after each meaningful successful inspection and labels that control **Continue Lesson**. Click **Pause** to preserve the selected run's durable checkpoint or **Stop** to end it. If a tool is already executing, Pause takes effect after that tool settles. **Return to Manual** first pauses queued or running work and waits for its durable checkpoint, then changes mode. If checkpointing fails, Radar remains AI-First and keeps Stop available in both windows. If the selected run has spent its runtime or tool-call budget, Radar shows the exact usage, disables Resume, and offers **Continue as New Run**. That explicit action starts the same goal and profile with a fresh bounded budget; it does not rewrite the exhausted checkpoint, and the new timeline records the source run id.
...
- The planner can advance the Mission Graph only through a bounded patch based on the current revision. Malformed, stale, invalid-reference, or unsupported-evidence planner patches are ignored as a whole and receive a visible **Mission update ignored; action continues** audit entry; the selected valid tool or finish action continues without partially updating the graph. Invalid operator mutations remain strict and fail closed.
...
- A gated action outside the narrow auto-granted navigation class creates a Radar-derived draft and pauses with the exact pending call before dispatch. **Approve Once** keeps the exact path and one-use bounds. **Approve All** covers matching calls only for the same tool, origin, method, and identity, remains profile- and budget-capped, and is invalidated by Scope or auth drift. The default-on **Resume after approval** control saves the grant, revalidates the ordinary resume guards, and queues the saved pending call exactly once. Clear it to keep the run paused.
...
- AI-First findings are quality-gated before entering **06 Findings**. Draft findings must include evidence references, affected assets, reproduction notes, severity rationale, remediation, and uncertainty notes. Every reference must also resolve to current local evidence; network-bearing evidence must remain in saved Scope. Rejected drafts remain counted in the Completion Report and visible as transcript cards, but do not enter the durable findings inbox.
...
- Tool calls, policy blocks, results, checkpoints, Mission Graph revisions, capability ledgers/receipts, and findings are saved locally with the active session and remain selectable through **Task History**.
...
7. For a lease proposal, Radar opens an exact-bounds permission dialog over the companion. Review the tuple, tool, risk tier, duration, and caps, then choose **Approve Once**, profile-capped **Approve All** for matching calls on the same origin, or **Deny** without searching the transcript. Radar keeps **Resume** unavailable while the lease is still a draft. **Resume after approval** is selected by default, so approval records authority and queues the saved pending call after the normal safety checks. Clear it if you want to remain paused and click **Resume** separately later.
...
### Managed Chrome Shows `ERR_PROXY_CONNECTION_FAILED`

This means the visible Chrome process is configured for a loopback proxy port that no longer has a listener. It can happen when Radar is interrupted or restarted without allowing its managed Chrome process to exit. A failed **Navigate** action retries once by rebuilding the managed browser automatically. **Open Browser** performs the same recovery directly: Radar verifies any stale process against the exact Radar profile ownership record, closes only that owned instance, and relaunches Chrome against the current proxy. In **12 SSL**, confirm that the displayed proxy state is **running** and that its URL matches Chrome's active Radar session. If Chrome cannot release the profile, close that managed Chrome window and retry **Open Browser**; normal personal Chrome profiles are unrelated.
...
- The selected **Task History** item is the run you intend to inspect.
