...
| C7. Agentic runtime foundation | Complete | Removed implicit scope widening from AI goals: out-of-scope origins populate the unsaved Scope editor and require Commit plus Start Run again. Added selectable saved run history; durable pause/resume checkpoints with cumulative step, replay, workflow-request, and elapsed-time budgets; operator recovery after failed tools; operator-requested safe/idempotent retries that exclude mutating browser, authentication-state, replay, and workflow actions; and finding gates that resolve every reference against current local, in-scope evidence before persistence. |
| C8. Persistent Mission Graph and operator steering | Complete | Added a versioned, revision-checked Mission Graph persisted locally per run with objectives, hypotheses, experiments, claims, coverage cells, and operator questions. The planner applies bounded patches rather than replacing the graph; stale revisions, invalid entity links, illegal operator mutations, and unresolved evidence fail closed. Supported hypotheses, supported/verified claims, and covered cells require locally resolvable evidence. Planner questions pause before tool execution. After pausing, operators can add objective/hypothesis branches, set legal statuses and priorities, pin hypotheses, and ask/answer/dismiss questions before resume; completed/stopped graphs are read-only, and Run History swaps the selected graph. |
| C9. Risk-tiered autonomy envelopes and capability leases | Complete | Added a locally persisted, per-run revisioned lease ledger where effective authority is the intersection of profile ceiling, saved Scope, an exact origin/method/path-prefix/identity tuple, and remaining run budgets. Passive/prepare tools remain lease-free; navigate, reversible, and active browser/auth/replay/active-workflow tools are gated; destructive and `DELETE` authority are never grantable. Planner drafts pause with the pending call before dispatch, Grant never resumes automatically, and Resume attempts that call once. Duration, actions, known explicit requests, concurrency, and payload are capped. Durable receipts reserve before dispatch and finalize outcomes; expiry/exhaustion, scope/auth drift, unexpected URL/auth/outcome, stop/completion, and runtime/session change revoke authority. |
| C10. Identity Lab and causal browser/network evidence | Complete | Added stable workspace-scoped identity profiles with explicit create/edit/activate/verify/archive controls and persistent per-identity Chrome user-data directories switched sequentially. Renderer and AI identity context is metadata-only; raw cookies/storage remain main-process/browser-profile data behind the existing raw-context opt-in, and insecure legacy global snapshots are disabled rather than treated as isolation or auto-imported. A persistent managed-Chrome CDP observer stamps request-time run/navigation/action/identity/activation lineage when available and preserves it across late updates; sequence/experiment fields are reserved for later producers and proxy-only traffic remains unattributed. The causal contract records exact/correlated/inferred/unmatched confidence and reasons, while the current UI calls action-tagged requests observations under an action context rather than proof of causation. Identity Lab provides health observations, an operator-labeled role × tenant × resource matrix using attributed captures only, and recorded-only one-identity differentials that send no traffic; 2xx remains an observation, not authorization proof. DOM snapshot persistence and active differential execution are not claimed in this slice. |
...
Phase A through Phase G and C10 are complete. Continue with **C11: independent verification and finding-to-regression**.
