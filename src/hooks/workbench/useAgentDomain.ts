...
export function useAgentDomain(ports: AgentDomainPorts) {
  ...
  const startAgentRun = useCallback(async (address: string) => {
    if (!window.radar) {
      ports.setNotice("Run in Electron to start an agent run.");
      return;
    }
    if (executingAgentRun) {
      setSelectedAgentRunId(executingAgentRun.id);
      ports.setNotice("An AI-First run is already active. Pause or stop it before starting another run.");
      return;
    }
    const goal = agentGoal.trim();
    if (!goal) {
      ports.setNotice("Describe a goal before starting AI-First.");
      return;
    }
    ...
    if (goalUrl && scopeOrigin) {
      ...
      if (!isAllowedTarget(goalUrl, latestTargets)) {
        const draftTargets = ports.targetText
          .split("\n")
          .map((item) => item.trim())
          .filter(Boolean);
        const proposedTargets = [...new Set([...latestTargets, ...draftTargets, scopeOrigin])];
        ports.setTargetText(proposedTargets.join("\n"));
        ports.setActiveView("scope");
        ports.setNotice(
    ...
    return run;
  }, [agentGoal, agentProfileId, agentTutorialMode, executingAgentRun, ports]);
  ...
  const pauseAgentRun = useCallback(async () => {
    ...
    try {
      ...
      if (run) {
        setAgentRuns((items) => [run, ...items.filter((item) => item.id !== run.id)]);
        ports.setNotice("AI-First run paused with budgets and checkpoint preserved.");
      }
    } catch (error) {
      ports.setNotice(error instanceof Error ? error.message : "Agent run could not be paused.");
    }
  }, [activeAgentRun, ports]);
  ...
  const resumeAgentRun = useCallback(async () => {
    ...
    try {
      ...
      if (run) {
        setAgentRuns((items) => [run, ...items.filter((item) => item.id !== run.id)]);
        ports.setNotice("AI-First run queued from its durable checkpoint.");
      }
    } catch (error) {
      ports.setNotice(error instanceof Error ? error.message : "Agent run could not be resumed.");
    }
  }, [activeAgentRun, ports]);

  const continueAgentRun = useCallback(async (address: string) => {
    if (!window.radar || !activeAgentRun) {
      return;
    }
    if (executingAgentRun) {
      setSelectedAgentRunId(executingAgentRun.id);
      ports.setNotice("An AI-First run is already active. Pause or stop it before starting a continuation.");
      return;
    }
    ...
    if (startUrl && scopeOrigin) {
      ...
      if (!isAllowedTarget(startUrl, latestTargets)) {
        const draftTargets = ports.targetText
          ...
          .map((item) => item.trim())
          .filter(Boolean);
        ports.setTargetText([...new Set([...latestTargets, ...draftTargets, scopeOrigin])].join("\n"));
        ports.setActiveView("scope");
        ports.setNotice(
    ...
    try {
      ...
      setAgentRuns((items) => [run, ...items.filter((item) => item.id !== run.id)]);
      setSelectedAgentRunId(run.id);
      ports.setNotice(`Continuation ${run.id.slice(0, 8)} started with a fresh bounded budget. ${sourceRun.id.slice(0, 8)} remains preserved.`);
    } catch (error) {
      ports.setNotice(error instanceof Error ? error.message : "A continuation run could not be started.");
    }
  }, [activeAgentRun, executingAgentRun, ports]);
    ...
    async (entryId: string, action: AgentRunRecoveryAction) => {
      ...
      try {
        ...
        if (action === "draft-finding") {
          ...
          ports.setNotice("Draft-finding prompt prepared from the selected failed step.");
          return;
        }
        ports.setNotice(
        ...
        ports.setNotice(error instanceof Error ? error.message : "Agent recovery could not be started.");
      }
    },
    [activeAgentRun, stopAgentRun, ports]
  );
  ...
  const steerAgentMission = useCallback(
    async (action: AgentMissionSteeringAction) => {
      ...
      if (!window.radar || !run?.mission) {
        ports.setNotice("Select a saved AI-First run with a Mission Graph before steering it.");
        return;
      }
      if (run.status !== "paused" && run.status !== "failed") {
        ports.setNotice("Pause the run and wait for the active step to settle before steering its Mission Graph.");
        return;
      }
      ...
      try {
        ...
        if (steered) {
          ...
          ports.setNotice(`Mission Graph updated to revision ${steered.mission?.revision ?? run.mission.revision}.`);
        }
      } catch (error) {
  ...
  const updateAgentCapabilities = useCallback(
    async (action: AgentCapabilityAction) => {
      const run = activeAgentRun;
      if (!window.radar || !run) {
        ports.setNotice("Select a saved AI-First run before changing capability leases.");
        return;
      }
      if (run.status !== "paused" && run.status !== "failed") {
        ports.setNotice("Pause the run and wait for the active step to settle before changing capability leases.");
        return;
      }
      ...
      try {
        ...
        if (updated) {
          ...
          ports.setNotice(`Capability ledger updated to revision ${updated.capabilities?.revision ?? expectedRevision}.`);
        }
      } catch (error) {
  ...
  const deleteAgentRunMemory = useCallback(async (entryId: string) => {
    if (!window.radar?.deleteAgentRunMemory) {
      ports.setNotice("Run in Electron to delete run memory.");
      return null;
    }
    const result = await window.radar.deleteAgentRunMemory(entryId);
    setAgentRunMemory(result.memory);
    ports.setNotice("Run memory deleted");
    return result;
  }, [ports]);

  // Agent timeline mirror effect
  useEffect(() => {
    if (ports.appMode !== "ai-first" || !activeAgentRun) {
    ...
    for (const entry of nextEntries) {
      ...
      if (appliesVisibleToolCall && entry.toolCall?.tool === "showView") {
        ports.setActiveView(entry.toolCall.input.view);
      }

      if (appliesVisibleToolCall && entry.toolCall?.tool === "sendReplay") {
        ports.setDraft(entry.toolCall.input.draft);
        ports.setHeadersText(formatHeaders(entry.toolCall.input.draft.headers));
        ports.setLastBurst(null);
      }

      if (entry.toolResult?.tool === "sendReplay" && entry.toolResult.ok) {
        ports.setLastResponse(entry.toolResult.data);
      }
      ...
      if (entry.toolResult?.tool === "getCaptures" && entry.toolResult.ok) {
        ...
        if (firstCapture) {
          ports.setSelectedId(firstCapture.id);
          ports.setSelectedIds([firstCapture.id]);
          ports.selectionAnchorRef.current = firstCapture.id;
        }
      }
      ...
      if (entry.toolResult?.tool === "getInterceptQueue" && entry.toolResult.ok) {
        ...
        ports.setInterceptState((current) => ({ ...current, queue }));
        const firstItem = queue[0];
        if (firstItem) {
          ports.hydrateInterceptDraft(firstItem);
        }
      }
      ...
      if (entry.toolResult?.tool === "prepareTrafficQuery" && entry.toolResult.ok) {
        ports.setTrafficSearch(entry.toolResult.data.query);
        ports.setActiveView("traffic");
        ports.setNotice(entry.toolResult.data.reason);
      }
      ...
      if (entry.toolResult?.tool === "prepareAutomateDraft" && entry.toolResult.ok) {
        ...
        ports.setDraft(preparedDraft);
        ports.setHeadersText(formatHeaders(preparedDraft.headers));
        ports.setAutomatePayloadText(payloads.join("\n"));
        ports.setAutomateRulesText(JSON.stringify(rules, null, 2));
        ports.setAutomateSessionName(name);
        ports.setLastResponse(null);
        ports.setLastBurst(null);
      ...
      if (entry.toolResult?.tool === "prepareWorkflowDraft" && entry.toolResult.ok) {
        ports.setAiPreparedWorkflowDraft(entry.toolResult.data.workflow);
        ports.setSelectedWorkflowId(entry.toolResult.data.workflow.id);
        ports.setActiveView("workflows");
        ports.setNotice(entry.toolResult.data.note);
      }

      if (entry.toolResult?.tool === "proposeRunMemory" && entry.toolResult.ok) {
        ports.setNotice(`AI proposed run memory: ${entry.toolResult.data.memory.title}`);
      }

      if (entry.toolResult?.tool === "analyzeAutomateResults" && entry.toolResult.ok) {
        ports.setActiveAutomateSessionId(entry.toolResult.data.sessionId);
        ports.setAutomateResultFilter(entry.toolResult.data.outlierResultIds.length > 0 ? "outliers" : "matches");
        ports.setActiveView("automate");
        ports.setNotice(
