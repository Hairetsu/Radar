...
export function createRadarApiStub(
  overrides: Partial<RadarApi> = {}
) {
const radarApi = {
  ...
  listIdentityProfiles: vi.fn(async () => []),
  createIdentityProfile: vi.fn(async (draft) => ({
    ...draft,
    id: "identity-test",
    workspaceId: "workspace-test",
    isolation: "dedicated-profile" as const,
    health: "unknown" as const,
    refreshMode: "manual" as const,
    jarRevision: 0,
    containerId: "container-identity-test",
    createdAt: "2026-07-10T12:00:00.000Z",
    updatedAt: "2026-07-10T12:00:00.000Z"
  })),
  updateIdentityProfile: vi.fn(async () => { throw new Error("Identity not found."); }),
  activateIdentityProfile: vi.fn(async () => { throw new Error("Identity not found."); }),
  verifyIdentityProfile: vi.fn(async () => { throw new Error("Identity not found."); }),
  archiveIdentityProfile: vi.fn(async () => ({ ok: true, identities: [] })),
  listIdentityActivations: vi.fn(async () => []),
  onCapture: vi.fn(() => () => undefined),
  onSslEvent: vi.fn(() => () => undefined),
  onBrowserState: vi.fn(() => () => undefined),
  navigateBrowser: vi.fn(async (url: string) => ({
    open: true,
    url,
    title: "Radar target",
    loading: false,
    engine: "chrome" as const,
    automation: "ready" as const,
  ...
  browserBack: vi.fn(async () => ({ open: true, url: "https://example.test/back", title: "Back", loading: false, engine: "chrome" as const, automation: "ready" as const })),
  browserForward: vi.fn(async () => ({ open: true, url: "https://example.test/forward", title: "Forward", loading: false, engine: "chrome" as const, automation: "ready" as const })),
  browserReload: vi.fn(async () => ({ open: true, url: "https://example.test", title: "Reloaded", loading: false, engine: "chrome" as const, automation: "ready" as const })),
  ...
  pauseAgentRun: vi.fn(async () => null),
  resumeAgentRun: vi.fn(async () => null),
